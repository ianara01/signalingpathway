...
# ---------------------------------------------------------------------
# Modeling / SBML post-analysis helpers
# ---------------------------------------------------------------------

def read_all_json_files(input_dir: Path) -> dict[str, Any]:
    """
    input_dir 내부의 모든 json 파일을 읽어서 {파일명: object} 형태로 반환한다.
    """
    ...
    for json_path in sorted(input_dir.glob("*.json")):
        json_tables[json_path.name] = read_json_if_exists(json_path, default={})

    return json_tables


def read_text_if_exists(path: str | Path, default: str = "") -> str:
    p = Path(path)
...
def load_modeling_results(modeling_dir: Path | None) -> dict[str, Any]:
    """
    results/modeling/hif1 형태의 후처리 모델링 결과 디렉토리를 읽는다.

    기대 파일 예:
      - HIF1A_target_classification.csv
      - VEGFA_target_classification.csv
      - ARNT_target_classification.csv
      - model_evidence.json
      - model_evidence_with_sbml.json
      - hif1_boolean_candidate.json
      - hif1_sbml_directory_summary.json
      - hif1_R-HSA-1234174_sbml_summary.json
      - post_modeling_summary.json
    """
    ...
    csv_tables = read_all_csv_files(modeling_dir)
    json_tables = read_all_json_files(modeling_dir)

    md_files: dict[str, str] = {}
    for md_path in sorted(modeling_dir.glob("*.md")):
        md_files[md_path.name] = read_text_if_exists(md_path)

    bnet_files = sorted(modeling_dir.glob("*.bnet"))
    bnet_text = read_text_if_exists(bnet_files[0]) if bnet_files else ""

    return {
        "dir": str(modeling_dir),
        "csv_tables": csv_tables,
        "json_tables": json_tables,
        "md_files": md_files,
        "bnet_text": bnet_text,
        "target_classification_tables": {
            name: df for name, df in csv_tables.items()
            if name.lower().endswith("_target_classification.csv")
        },
        "model_evidence": json_tables.get("model_evidence.json", {}),
        "model_evidence_with_sbml": json_tables.get("model_evidence_with_sbml.json", {}),
        "boolean_candidate": _find_json_by_patterns(json_tables, ["boolean", "candidate"]),
        "sbml_directory_summary": _find_json_by_patterns(json_tables, ["sbml", "directory", "summary"]),
        "single_sbml_summaries": {
            name: obj for name, obj in json_tables.items()
            if "sbml_summary" in name.lower() and "directory" not in name.lower()
        },
        "post_modeling_summary": json_tables.get("post_modeling_summary.json", {}),
    }


def _find_json_by_patterns(
    json_tables: dict[str, Any],
    patterns: list[str],
) -> Any:
    for filename, obj in json_tables.items():
        lower = filename.lower()
        if all(pattern.lower() in lower for pattern in patterns):
            return obj
    return {}
...
def _summarize_evidence_json(evidence: dict[str, Any]) -> dict[str, Any]:
    """
    model_evidence.json 또는 model_evidence_with_sbml.json의 target별 판정을 요약한다.
    """
    ...
    evidence_flag_counts: dict[str, int] = {}
    ...
    for target, ev in evidence.items():
        ...
        for flag, value in flags.items():
            if bool(value):
                evidence_flag_counts[flag] = evidence_flag_counts.get(flag, 0) + 1

        target_summaries.append({
            "target": target,
            "match_count": len(ev.get("target_matches", []) or []),
            "model_types": "; ".join(ev.get("model_types", []) or []),
            "topology_graph_model": bool(flags.get("topology_graph_model", False)),
            "signed_regulatory_model_candidate": bool(flags.get("signed_regulatory_model_candidate", False)),
            "target_signed_regulatory_model_candidate": bool(flags.get("target_signed_regulatory_model_candidate", False)),
            "boolean_model_candidate": bool(flags.get("boolean_model_candidate", False)),
            "reactome_reaction_event_model": bool(flags.get("reactome_reaction_event_model", False)),
            "stoichiometric_like_model_candidate": bool(flags.get("stoichiometric_like_model_candidate", False)),
            "sbml_candidate": bool(flags.get("sbml_candidate", False)),
            "sbml_structural_model_present": bool(flags.get("sbml_structural_model_present", False)),
            "kinetic_ode_model_evidence": bool(flags.get("kinetic_ode_model_evidence", False)),
            "conclusion": " / ".join(ev.get("conclusion", [])[:3]),
        })

    return {
        "target_count": len(target_summaries),
        "targets": [x["target"] for x in target_summaries],
        "evidence_flag_counts": evidence_flag_counts,
        "target_summaries": target_summaries,
    }


def _summarize_target_classification(csv_tables: dict[str, pd.DataFrame]) -> dict[str, Any]:
    classification_summaries: list[dict[str, Any]] = []

    for filename, df in csv_tables.items():
        if not filename.lower().endswith("_target_classification.csv"):
            continue

        keyword = filename.replace("_target_classification.csv", "")
        target_type_counts = value_counts_dict(df, "target_type")
        node_type_counts = value_counts_dict(df, "node_type")
        confidence_avg = None
        if not df.empty and "confidence" in df.columns:
            try:
                confidence_avg = float(pd.to_numeric(df["confidence"], errors="coerce").mean())
            except Exception:
                confidence_avg = None
        ...
        classification_summaries.append({
            ...
            "samples": df_to_records(df, 10),
        })

    return {
        "classification_file_count": len(classification_summaries),
        "classification_summaries": classification_summaries,
    }


def _summarize_boolean_candidate(boolean_candidate: dict[str, Any], bnet_text: str) -> dict[str, Any]:
    if not isinstance(boolean_candidate, dict):
        boolean_candidate = {}

    rules = boolean_candidate.get("rules", {}) or {}
    records = boolean_candidate.get("records", []) or []
    signed_edges = boolean_candidate.get("signed_edges", []) or []
    limitations = boolean_candidate.get("limitations", []) or []

    bnet_rule_count = 0
    if bnet_text:
        lines = [line for line in bnet_text.splitlines() if line.strip()]
        bnet_rule_count = max(0, len(lines) - 1) if lines else 0

    return {
        "rule_count": len(rules) if isinstance(rules, dict) else len(records),
        "record_count": len(records),
        "signed_edge_count": len(signed_edges),
        "include_via_group": bool(boolean_candidate.get("include_via_group", False)),
        "expression_positive": bool(boolean_candidate.get("expression_positive", False)),
        "bnet_rule_count": bnet_rule_count,
        "rule_samples": records[:20] if isinstance(records, list) else [],
        "limitations": limitations,
        "inference": [
            "Boolean 후보 모델은 activation/inhibition/expression edge에서 유도된 topology 기반 논리 규칙이다.",
            "via_group을 포함한 경우 HIF1A→VEGFA 등 group expansion 기반 inferred relation이 Boolean rule에 들어갈 수 있으므로 직접 기전으로 단정하면 안 된다.",
            "rate, threshold, time delay, kinetic parameter가 없으므로 ODE/kinetic model이 아니라 Boolean-rule candidate로 해석해야 한다.",
        ],
    }


def _summarize_sbml_results(
    directory_summary: dict[str, Any],
    single_summaries: dict[str, Any],
) -> dict[str, Any]:
    if not isinstance(directory_summary, dict):
        directory_summary = {}

    file_summaries = directory_summary.get("summaries", []) or []
    ...
    for item in file_summaries:
        ...
        model_rows.append({
            ...
            "species_count": item.get("species_count", 0),
            "reaction_count": item.get("reaction_count", 0),
            "parameter_count": item.get("parameter_count", 0),
            "kinetic_law_count": item.get("kinetic_law_count", 0),
            "has_structural_model": item.get("has_structural_model", False),
            "kinetic_ode_model_candidate": item.get("kinetic_ode_model_candidate", False),
        })

    single_rows: list[dict[str, Any]] = []
    for filename, item in single_summaries.items():
        if not isinstance(item, dict):
            continue
        single_rows.append({
    ...
    structural_count = sum(1 for row in model_rows if row.get("has_structural_model"))
    kinetic_candidate_count = sum(1 for row in model_rows if row.get("kinetic_ode_model_candidate"))

    if kinetic_candidate_count > 0:
        inference = "SBML 중 kinetic/ODE 후보가 존재한다."
    elif structural_count > 0:
        inference = "SBML structural model은 존재하지만 kineticLaw/parameter 부족으로 executable ODE model로는 확인되지 않는다."
    else:
        inference = "SBML structural model이 확인되지 않았거나 SBML 파일이 없다."

    return {
        "sbml_file_count": int(directory_summary.get("sbml_file_count", 0) or 0),
        "total_species_count": int(directory_summary.get("total_species_count", 0) or 0),
        "total_reaction_count": int(directory_summary.get("total_reaction_count", 0) or 0),
        "total_parameter_count": int(directory_summary.get("total_parameter_count", 0) or 0),
        "total_kinetic_law_count": int(directory_summary.get("total_kinetic_law_count", 0) or 0),
        "any_kinetic_ode_model_candidate": bool(directory_summary.get("any_kinetic_ode_model_candidate", False)),
        "structural_model_file_count": structural_count,
        "kinetic_candidate_file_count": kinetic_candidate_count,
        "model_rows": model_rows,
        "single_summary_rows": single_rows,
        "inference": inference,
    }


def analyze_modeling_results(modeling: dict[str, Any]) -> dict[str, Any]:
    """
    results/modeling/hif1 후처리 결과를 분석한다.
    """
    if not modeling or not modeling.get("dir"):
        return {
            "available": False,
            "inference": ["modeling 결과 디렉토리가 제공되지 않아 후처리 분석은 수행되지 않았다."],
        }

    csv_tables = modeling.get("csv_tables", {}) or {}
    json_tables = modeling.get("json_tables", {}) or {}

    classification_summary = _summarize_target_classification(csv_tables)
    evidence_summary = _summarize_evidence_json(modeling.get("model_evidence", {}))
    evidence_with_sbml_summary = _summarize_evidence_json(modeling.get("model_evidence_with_sbml", {}))
    boolean_summary = _summarize_boolean_candidate(
        modeling.get("boolean_candidate", {}),
        modeling.get("bnet_text", ""),
    )
    sbml_summary = _summarize_sbml_results(
        modeling.get("sbml_directory_summary", {}),
        modeling.get("single_sbml_summaries", {}),
    )

    post_summary = modeling.get("post_modeling_summary", {}) or {}
    step_status_rows: list[dict[str, Any]] = []
    for step_name, step_info in (post_summary.get("steps", {}) or {}).items():
        if isinstance(step_info, dict):
            step_status_rows.append({
                "step": step_name,
                "status": step_info.get("status", ""),
                "output": step_info.get("output", step_info.get("json", step_info.get("md", step_info.get("output_dir", "")))),
            })

    inference = [
        "modeling 후처리 결과는 KEGG/Reactome graph 결과를 수학적 모델링 가능성 관점에서 재분류한다.",
        f"Target classification 파일 {classification_summary['classification_file_count']}개를 읽었다.",
        f"Boolean 후보 규칙 {boolean_summary.get('rule_count', 0)}개와 signed edge {boolean_summary.get('signed_edge_count', 0)}개가 확인되었다.",
        sbml_summary.get("inference", ""),
    ]

    if evidence_with_sbml_summary.get("target_count", 0):
        inference.append("SBML summary가 포함된 model_evidence_with_sbml.json이 있어 graph evidence와 SBML evidence를 함께 판단할 수 있다.")
    elif evidence_summary.get("target_count", 0):
        inference.append("model_evidence.json은 있으나 SBML summary가 결합된 evidence는 확인되지 않았다.")

    return {
        "available": True,
        "dir": modeling.get("dir"),
        "loaded_csv_files": sorted(csv_tables.keys()),
        "loaded_json_files": sorted(json_tables.keys()),
        "classification_summary": classification_summary,
        "model_evidence_summary": evidence_summary,
        "model_evidence_with_sbml_summary": evidence_with_sbml_summary,
        "boolean_model_summary": boolean_summary,
        "sbml_summary": sbml_summary,
        "post_modeling_steps": step_status_rows,
        "inference": [x for x in inference if x],
    }
...
def build_markdown_report(
    ...
    modeling_analysis: dict[str, Any] | None = None,
) -> str:
    lines: list[str] = []
    quality_flags = quality_flags or {}
    result_file_guide = result_file_guide or {"kegg": [], "reactome": []}
    modeling_analysis = modeling_analysis or {"available": False}
    ...
    # 10
    lines.append("## 10. Modeling / SBML 후처리 결과")
    lines.append("")
    if not modeling_analysis.get("available"):
        lines.append("- modeling 결과 디렉토리 또는 ZIP이 제공되지 않아 후처리 모델링 분석은 포함되지 않았습니다.")
    else:
        lines.append(f"- modeling result dir: {modeling_analysis.get('dir', '')}")
        lines.append(f"- loaded modeling CSV files: {', '.join(modeling_analysis.get('loaded_csv_files', []))}")
        lines.append(f"- loaded modeling JSON files: {', '.join(modeling_analysis.get('loaded_json_files', []))}")
        lines.append("")

        cls = modeling_analysis.get("classification_summary", {})
        lines.append("### 10.1 Target type classification")
        lines.append("")
        lines.append(f"- classification file count: {cls.get('classification_file_count', 0)}")
        for item in cls.get("classification_summaries", []):
            lines.append(f"#### {item.get('keyword', '')}")
            lines.append(f"- rows: {item.get('row_count', 0)}")
            lines.append(md_dict_counts("target_type counts", item.get("target_type_counts", {})))
            lines.append("")
            lines.append(render_records(item.get("samples", []), 5))
            lines.append("")

        ev = modeling_analysis.get("model_evidence_summary", {})
        ev_sbml = modeling_analysis.get("model_evidence_with_sbml_summary", {})
        lines.append("### 10.2 Model evidence")
        lines.append("")
        lines.append(f"- model_evidence targets: {', '.join(ev.get('targets', []))}")
        lines.append(f"- model_evidence_with_sbml targets: {', '.join(ev_sbml.get('targets', []))}")
        lines.append("")
        lines.append("#### Evidence with SBML target summaries")
        lines.append("")
        lines.append(render_records(ev_sbml.get("target_summaries", []) or ev.get("target_summaries", []), 10))
        lines.append("")

        boolean = modeling_analysis.get("boolean_model_summary", {})
        lines.append("### 10.3 Boolean model candidate")
        lines.append("")
        lines.append(f"- rule count: {boolean.get('rule_count', 0)}")
        lines.append(f"- signed edge count: {boolean.get('signed_edge_count', 0)}")
        lines.append(f"- include_via_group: {boolean.get('include_via_group', False)}")
        lines.append(f"- expression_positive: {boolean.get('expression_positive', False)}")
        lines.append("")
        lines.append("#### Boolean rule samples")
        lines.append("")
        lines.append(render_records(boolean.get("rule_samples", []), 10))
        lines.append("")
        for x in boolean.get("inference", []):
            lines.append(f"- {x}")
        lines.append("")

        sbml = modeling_analysis.get("sbml_summary", {})
        lines.append("### 10.4 SBML structural / kinetic evidence")
        lines.append("")
        lines.append(f"- SBML file count: {sbml.get('sbml_file_count', 0)}")
        lines.append(f"- total species count: {sbml.get('total_species_count', 0)}")
        lines.append(f"- total reaction count: {sbml.get('total_reaction_count', 0)}")
        lines.append(f"- total parameter count: {sbml.get('total_parameter_count', 0)}")
        lines.append(f"- total kineticLaw count: {sbml.get('total_kinetic_law_count', 0)}")
        lines.append(f"- any kinetic ODE model candidate: {sbml.get('any_kinetic_ode_model_candidate', False)}")
        lines.append(f"- inference: {sbml.get('inference', '')}")
        lines.append("")
        lines.append("#### SBML model summaries")
        lines.append("")
        lines.append(render_records(sbml.get("model_rows", []), 10))
        lines.append("")

        steps = modeling_analysis.get("post_modeling_steps", [])
        lines.append("### 10.5 Post-modeling workflow steps")
        lines.append("")
        lines.append(render_records(steps, 20))
        lines.append("")

        lines.append("### 10.6 Modeling interpretation")
        lines.append("")
        for x in modeling_analysis.get("inference", []):
            lines.append(f"- {x}")
        lines.append("")
...
def main() -> None:
    ...
    parser.add_argument("--modeling-zip", default=None)
    parser.add_argument("--modeling-dir", default=None)
    parser.add_argument(
        "--results-root",
        default="results",
    ...
    modeling_dir = resolve_or_find_input_dir(
        dir_path=args.modeling_dir,
        zip_path=args.modeling_zip,
        work_dir=work_dir,
        results_root=results_root,
        auto_pattern="modeling/hif1",
        label="Modeling HIF-1",
    )
    ...
    modeling_analysis = analyze_modeling_results(modeling)
    ...
    analysis_json = {
        ...
        "modeling_dir": str(modeling_dir) if modeling_dir else None,
